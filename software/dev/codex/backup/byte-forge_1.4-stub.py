...
def refresh_byte_tree():
    ...
    for byte_id, entry in byte_array_dict.items():
        ...
        size = entry["size"]
        data = entry["data"]
        byte_tree.insert("", "end", values=(str(byte_id), name, size, data))
...
def set_snapshot(snap, push_to_stack=False):
    ...
    # Disable trace for grid_size_var
    if hasattr(grid_size_var, 'trace_id'):
        print("[DEBUG] Disabling trace for grid_size_var in set_snapshot")
    ...
    try:
        ...
        # Re-enable trace for grid_size_var
        grid_size_var.trace_id = grid_size_var.trace_add("write", on_grid_size_change)
        print("[DEBUG] Re-enabled trace for grid_size_var in set_snapshot")
...
def show_byte_dropdown(iid, bbox): # there's a bug here where we are no longer referencing the right dict structure since we moved away from referencing name to unique id. we will still display the name in the dropdown, but internally referece the id
    idx = eq_tree.index(iid)
...
byte_tree.column("ID", width=0, stretch=False)  # Hide ID column
...
def update_byte_array():
    ...
    if current_byte_key:
        print(f"[DEBUG] Current byte_array_dict keys: {list(byte_array_dict.keys())}")
        print(f"[DEBUG] Current current_byte_key: {current_byte_key}")
        if current_byte_key in byte_array_dict:
            ...
            print(f"[ERROR] Key {current_byte_key} not found in byte_array_dict")
    else:
...
def on_byte_tree_select(event):
    ...
    byte_id = str(row_vals[0])  # Ensure byte_id is a string
    byte_name = row_vals[1]
    byte_text.delete(0, tk.END)
    byte_text.insert(0, byte_name)

    global current_byte_key
    if current_byte_key == byte_id:
        print(f"[DEBUG] Skipping re-selection for byte_id={byte_id}")
        return  # Prevent unnecessary updates

    current_byte_key = byte_id
    print(f"[DEBUG] Byte '{byte_id}' selected. Loading into pixel_data.")

    # Temporarily disable trace
    if hasattr(grid_size_var, 'trace_id'):
        grid_size_var.trace_vdelete("write", grid_size_var.trace_id)
        print("[DEBUG] Disabled trace for grid_size_var in on_byte_tree_select")
    ...
    try:
        ...
        # Re-enable trace
        grid_size_var.trace_id = grid_size_var.trace_add("write", on_grid_size_change)
        print("[DEBUG] Re-enabled trace for grid_size_var in on_byte_tree_select")
...
def on_byte_tree_double_click(event):
    ...
    byte_id = int(byte_tree.item(iid)["values"][0])  # ID is now internal
    bbox = byte_tree.bbox(iid, column=1)  # "Name" column
    if not bbox:
        return
    ...
    def save_rename(event=None):
        ...
        if not new_name:
            print("[DEBUG] New name is empty. Skipping rename.")
            return
        ...
        refresh_byte_tree()
        save_codex()
        print(f"[DEBUG] Renamed byte ID {byte_id} to '{new_name}'.")

    byte_edit_entry.bind("<Return>", save_rename)
    byte_edit_entry.bind("<FocusOut>", save_rename)
