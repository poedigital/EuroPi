        ...
        with open(CODEX_FILE, "r", encoding="utf-8") as f:
            try:
                ...
                operator_pool = data.get("operator_pool", {
                    ...
                    "constants": {},
                    "groupings": [],
                    "variables": {}  # Ensure variables is a dictionary
                })

                # If operator_pool["variables"] is not a dictionary, fix it
                if not isinstance(operator_pool.get("variables"), dict):
...
def save_codex():
    try:
        ...
        print(f"[ERROR] Failed to save Codex: {e}")
...
def set_snapshot(snap, section="byte", push_to_stack=False):
    ...
    if section == "byte":
        global byte_array_dict, pixel_data, current_byte_key  # declare once at top

        # Restore dictionary & pixel data
        byte_array_dict = copy.deepcopy(snap.get("byte_array_dict", {}))
        pixel_data = copy.deepcopy(snap.get("pixel_data", [[0]*MAX_DIM for _ in range(MAX_DIM)]))
        current_byte_key = snap.get("current_byte_key", None)
        grid_size_var.set(snap.get("grid_size", 8))  # Ensure UI reflects snapshot
...
def pop_undo(section="byte"):
    if section == "byte":
        if not byte_undo_stack:
            return
        current_state = copy.deepcopy(get_snapshot("byte"))  # Ensure deep copy!
        previous_state = byte_undo_stack.pop()
        byte_redo_stack.append(current_state)  # Store full deep copy
...
def refresh_equation_tree():
    ...
    for e in equation_dict:
        ...
        rpn_debug = " ".join(e.get("rpn", [])) if "rpn" in e else ""  # show rpn in debug
        iid = eq_tree.insert("", "end", values=(i, t, f, v, b, rpn_debug))
...
def on_eq_click(event):
    ...
    if new_index != old_index:
        ...
        update_oled_equation()
        refresh_ranges_tree()
        tokenize_equation(new_index)  # Populate tokenizer
...
def tokenize_equation(eq_idx=None):
    """
    Tokenizes the equation by simple space splitting, categorizes tokens, 
    and runs infix_to_rpn() for final RPN. 
    Preserves clumped tokens (e.g. b(c-d)) unless the user adds spaces.
    """
    # 1) Determine which equation we are working on
    if eq_idx is None:
        sel = eq_tree.selection()
        if not sel:
            print("[DEBUG] No equation selected.")
            return
        eq_idx = eq_tree.index(sel[0])

    if eq_idx < 0 or eq_idx >= len(equation_dict):
        # If invalid index, clear tokenizer UI
        tokenizer_entry.delete(0, tk.END)
        tokenizer_tree.delete(*tokenizer_tree.get_children())
        return

    # 2) Retrieve and prepare the equation text
    eq_data = equation_dict[eq_idx]
    equation_text = eq_data.get("equation", "").strip()

    # 3) Handle empty equation gracefully
    if not equation_text:
        tokenizer_entry.delete(0, tk.END)
        tokenizer_tree.delete(*tokenizer_tree.get_children())
        return

    # 4) Apply any 'exceptions' substitutions and show in tokenizer_entry
    parsed_eq = parse_exceptions(equation_text)
    tokenizer_entry.delete(0, tk.END)
    tokenizer_entry.insert(0, parsed_eq)

    # 5) Split into raw tokens (space-based)
    raw_tokens = parsed_eq.split()
    tokenizer_tree.delete(*tokenizer_tree.get_children())

    print(f"\n[DEBUG] Tokenizing Equation: '{parsed_eq}'")

    tokens = []
    variables = []

    # 6) Classify each token with a simple category check
    for token in raw_tokens:
        if token in operator_pool["functions"]:
            category = "function"
        elif token in operator_pool["operators"]:
            category = "operator"
        elif token in operator_pool["constants"]:
            category = "constant"
        elif token in operator_pool["groupings"]:
            category = "grouping"
        elif token in operator_pool["variables"]:
            category = "variable"
            variables.append(token)
        else:
            category = "unknown"

        # Insert token into tokenizer_tree, with '???' if unknown
        assigned_token = token if category != "unknown" else "???"
        tokenizer_tree.insert("", "end", values=(token, category, assigned_token))
        tokens.append(token)

        print(f"  [DEBUG] → '{token}' categorized as '{category}'")

    # 7) Update eq_data: tokens, vars, equation
    eq_data["tokens"] = tokens
    eq_data["vars"] = list(set(variables))  # deduplicate
    eq_data["equation"] = parsed_eq  # store the final parsed text if you want

    # 8) Generate RPN from these tokens
    rpn_output = infix_to_rpn(tokens, operator_pool)
    eq_data["rpn"] = rpn_output
    print("[DEBUG] Final RPN Output:", rpn_output)
...
tk.Button(tokenizer_input_frame, text="Tokenize", command=tokenize_equation).pack(side="left", padx=2)

def infix_to_rpn(tokens, operator_pool):
    precedence = {op: 1 for op in operator_pool["operators"]}
    precedence.update({"*": 2, "⋅": 2, "/": 2, "^": 3})
    precedence.update({fn: 4 for fn in operator_pool["functions"]})

    output = []
    operator_stack = []
    function_stack = []  # Track function application

    print("\n[DEBUG] Starting infix_to_rpn")
    print(f"[DEBUG] Tokens received: {tokens}")

    for token in tokens:
        if token == "=":  # Skip assignment operator
            print(f"[DEBUG] Skipping '=' (assignment operator, not needed in RPN)")
            continue

        elif token in operator_pool["functions"]:
            operator_stack.append(token)  # Push function onto stack
            function_stack.append(token)  # Track function application
            print(f"[DEBUG] Function '{token}' pushed to stack and function_stack")
            ...
            if function_stack and operator_stack and operator_stack[-1] in function_stack:
...
def on_tokenizer_select(event):
    """Handles token selection and selects the matching text inside tokenizer_entry."""
    sel = tokenizer_tree.selection()
    if not sel:
        return
    ...
    # Get current text from tokenizer_entry
    entry_text = tokenizer_entry.get()

    # Find the first occurrence of the selected token
    start_idx = entry_text.find(raw_text)

    if start_idx != -1:
        end_idx = start_idx + len(raw_text)

        # Ensure the entry field is focused before selecting
        tokenizer_entry.focus_set()

        # Select the token in tokenizer_entry
        tokenizer_entry.selection_range(start_idx, end_idx)
        tokenizer_entry.icursor(end_idx)
...
def on_tokenizer_double_click(event):
    """ triggers add_operator window if the selected token has '???' as assigned. """
    sel = tokenizer_tree.selection()
    if not sel:
        return
    ...
    # only trigger add_operator if the token is unknown (???)
    if assigned_token == "???":
        add_operator(raw_text)
...
operator_cols = ("Category", "Symbol/Key", "Value", "Expandable", "Expansion")
operator_tree = ttk.Treeview(
    operator_section.content, columns=operator_cols, show="headings", height=8
)

operator_tree.column("Category", width=100, anchor="w", stretch=False)
operator_tree.column("Symbol/Key", width=120, anchor="w", stretch=False)
operator_tree.column("Value", width=80, anchor="center", stretch=False)
operator_tree.column("Expandable", width=80, anchor="center", stretch=False)
operator_tree.column("Expansion", width=200, anchor="w", stretch=True)  # Allow expansion column to stretch
...
def refresh_operator_tree():
    """Refreshes the operator tree with updated data."""
    ...
    for category, items in operator_pool.items():
        ...
        elif category == "variables" and isinstance(items, dict):
            for symbol, details in items.items():
                expandable = details.get("expandable", False)
                expansion = " ".join(details.get("expansion", []))
                operator_tree.insert("", "end", values=(category, symbol, "", str(expandable), expansion))
        elif isinstance(items, list):
            for symbol in items:
                operator_tree.insert("", "end", values=(category, symbol, "", "", ""))

def add_operator(event=None):
    ...
    # Force update to get widget dimensions
    add_window.update_idletasks()

    # Get dropdown widget position relative to the window
    dropdown_x = category_dropdown.winfo_rootx() - add_window.winfo_rootx()
    dropdown_y = category_dropdown.winfo_rooty() - add_window.winfo_rooty()

    # Adjust window position so the dropdown aligns with the cursor
    new_x = cursor_x - dropdown_x - 300 # shift window left by 30px
    new_y = cursor_y - dropdown_y - 40
    add_window.geometry(f"+{new_x}+{new_y}")
    ...
    # Expandable Checkbox (Only visible for variables)
    expandable_var = tk.BooleanVar(value=False)
    expandable_check = tk.Checkbutton(add_window, text="Expandable", variable=expandable_var)
    expandable_check.grid(row=3, column=0, columnspan=2, pady=5)

    tk.Label(add_window, text="Expansion (comma-separated tokens):").grid(row=4, column=0, padx=5, pady=5)
    expansion_entry = tk.Entry(add_window)
    expansion_entry.grid(row=4, column=1, padx=5, pady=5)

    def confirm_add():
        category = category_var.get()
        symbol = symbol_entry.get().strip()
        value = value_entry.get().strip()
        expandable = expandable_var.get()
        expansion = expansion_entry.get().strip().split(",") if expandable else []
        ...
        if category not in operator_pool:
            if category == "constants":
                operator_pool[category] = {}  # Ensure it's a dictionary
            else:
                operator_pool[category] = []  # Ensure it's a list
        ...
        if category == "constants":
            ...
            operator_pool["variables"][symbol] = {
...
def on_operator_double_click(event):
    """Handles inline editing of operator pool entries on double-click."""
    sel = operator_tree.selection()
    if not sel:
        return
    ...
    if c_index < 1:  # Only allow editing for specific columns
        return

    values = operator_tree.item(iid, "values")
    if len(values) < 5:
        return  # Ensure valid selection

    category, symbol, value, expandable, expansion = values

    # **1. Prevent editing expandable for non-variables**
    if c_index == 3:  # Expandable Column
        if category != "variables":  # Only allow variables to have expandable properties
            print(f"[DEBUG] Expandable flag is not editable for category '{category}'")
            return

        if symbol not in operator_pool["variables"]:
            print(f"[WARNING] Attempted to toggle expandable for '{symbol}', but it's not found.")
            return

        new_state = not operator_pool["variables"][symbol].get("expandable", False)
        operator_pool["variables"][symbol]["expandable"] = new_state

        # update treeview with new boolean value
        operator_tree.item(iid, values=(category, symbol, value, str(new_state), expansion))
        save_codex()
        return

    # **2. Only allow variables to edit expansion**
    if c_index == 4:  # Expansion Column
        if category != "variables":
            print(f"[DEBUG] Expansion is only editable for variables (attempted on '{category}')")
            return

        edit_entry = tk.Entry(operator_tree)
        edit_entry.place(x=operator_tree.winfo_rootx(), y=operator_tree.winfo_rooty(), width=200, height=20)
        edit_entry.insert(0, expansion)
        edit_entry.focus()

        def save_expansion(event=None):
            """Saves the edited expansion sequence."""
            new_expansion = edit_entry.get().strip().split()
            edit_entry.place_forget()

            if symbol in operator_pool["variables"]:
                operator_pool["variables"][symbol]["expansion"] = new_expansion
                operator_tree.item(iid, values=(category, symbol, value, expandable, " ".join(new_expansion)))

            save_codex()

        edit_entry.bind("<Return>", save_expansion)
        edit_entry.bind("<FocusOut>", save_expansion)
        return

    # **3. Only allow constants to edit values**
    if c_index == 2:  # Value Column
        if category != "constants":  # Prevent editing values for non-constants
            print(f"[DEBUG] Values are only editable for constants (attempted on '{category}')")
            return

    # **4. Inline editing for symbol & value (constants only)**
    edit_entry = tk.Entry(operator_tree)
    ...
    def save_edit(event=None):
        ...
        if c_index == 1:  # Editing Symbol/Key
            ...
            if new_value in operator_pool[category]:
                ...
                return
            operator_pool[category][new_value] = operator_pool[category].pop(symbol)

        elif c_index == 2:  # Editing Value (only for constants)
            try:
        ...
        operator_tree.item(iid, values=(category, symbol, new_value, expandable, expansion))
        save_codex()
...
def parse_exceptions(equation):
    ...
    # ensure exceptions exist and clean up empty mappings
    exceptions = {k: v for k, v in exceptions.items() if k.strip()}
    if exceptions:
        pattern = re.compile("|".join(re.escape(k) for k in exceptions))
        equation = pattern.sub(lambda match: exceptions.get(match.group(0), ""), equation)

    # normalize excessive spaces
    equation = re.sub(r'\s{2,}', ' ', equation)  # replaces 2+ spaces with a single space
...
def init():
    ...
    refresh_exceptions_tree()
    refresh_operator_tree()
    print("[DEBUG] Loaded Operator Pool Variables:", json.dumps(operator_pool["variables"], indent=2))
