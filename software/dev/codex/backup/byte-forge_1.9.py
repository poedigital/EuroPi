    ...
    if os.path.exists(CODEX_FILE):
        with open(CODEX_FILE, "r", encoding="utf-8") as f:
            try:
                ...
                operator_pool = data.get("operator_pool", {"operators": [], "functions": [], "constants": [], "groupings": [], "variables": []})
                return equation_dict, byte_array_dict, operator_pool
            except Exception as e:
                print(f"[ERROR] Failed to load Codex: {e}")
                return [], {}, {"operators": [], "functions": [], "constants": [], "variables": []}
    return [], {}, {"operators": [], "functions": [], "constants": [], "variables": []}
...
def split_token(row_id):
    row = tokenizer_tree.item(row_id, "values")
    raw_text, category, _ = row

    # Split based on spaces or user input
    ...
    for token in tokens:
        ...
        tokenizer_tree.insert("", "end", values=(token, cat, token if cat != "unknown" else "???"))

def tokenize_equation():
    """tokenizes the equation while ensuring exception parsing and preserving focus."""
    sel = eq_tree.selection()
    if not sel:
        print("[debug] no equation selected.")
        return

    eq_idx = eq_tree.index(sel[0])  # save selected equation index

    # first, ensure the equation is saved to the dictionary before parsing
    equation_dict[eq_idx]["equation"] = tokenizer_entry.get().strip()

    eq = equation_dict[eq_idx]["equation"]
    ...
    equation_dict[eq_idx]["equation"] = " ".join(tokens)
    equation_dict[eq_idx]["vars"] = list(set(variables))  # remove duplicates

    # save immediately to ensure persistence
    save_codex()

    # refresh but restore selection
    refresh_equation_tree()
...
def on_tokenizer_double_click(event):
    ...
    if assigned_token == "???":
        add_operator_from_token(raw_text)
tokenizer_tree.bind("<Double-1>", on_tokenizer_double_click)

def add_operator_from_token(symbol):
    """ opens add_operator with pre-filled values from tokenizer. """
    ...
    category_var = tk.StringVar(value="operators")  # default to operators
    ...
    symbol_entry.insert(0, symbol)  # pre-fill with token's raw text
    ...
    def confirm_add():
        ...
        # apply exception parsing before adding
        ...
        tokenize_equation()  # re-tokenize to update categorization
        add_window.destroy()
...
def add_operator(event=None):
    """Opens a pop-up to add a new operator to the pool, positioned near the cursor."""
    add_window = tk.Toplevel(root)
    add_window.title("Add Operator")
    ...
    def confirm_add():
        ...
        # Apply exception parsing to normalize the symbol (e.g., "*" → "⋅")
        symbol = parse_exceptions(symbol)

        # Ensure category exists with the correct type
        if category not in operator_pool:
            if category == "constants":
        ...
        if category == "constants":
            if not isinstance(operator_pool[category], dict):
                operator_pool[category] = {}  # Fix incorrect data type if necessary
