...
def refresh_operator_tree():
    ...
    for category, items in operator_pool.items():
        if isinstance(items, dict):  # For dictionaries (constants and variables)
            for symbol, value in items.items():
                # If it's a variable (which is an empty dict), display it without a value
                if category == "variables":
                    operator_tree.insert("", "end", values=(category, symbol, ""))
                else:  # Constants have values
                    operator_tree.insert("", "end", values=(category, symbol, value))
        
        elif isinstance(items, list):  # For lists (operators, functions, groupings)
            for symbol in items:
                operator_tree.insert("", "end", values=(category, symbol, ""))
...
def add_operator(event=None):
    ...
    new_x = cursor_x - dropdown_x
    new_y = cursor_y - dropdown_y
