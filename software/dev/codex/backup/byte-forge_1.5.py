...
def validate_byte_array_dict(data):
    valid_dict = {}
    for byte_id, entry in data.items():
        try:
            name = entry.get("name", f"Byte_{byte_id}")
            size = int(entry["size"])
            data_list = entry["data"]
            if not isinstance(data_list, list):
                raise ValueError("Invalid data format")
            valid_dict[str(byte_id)] = {"name": name, "size": size, "data": data_list}
        except (KeyError, ValueError, TypeError) as e:
            print(f"[DEBUG] Invalid byte entry with ID {byte_id}: {entry}. Error: {e}")
    return valid_dict
...
def load_codex():
    if os.path.exists(CODEX_FILE):
        with open(CODEX_FILE, "r", encoding="utf-8") as f:
            try:
                data = json.load(f)
                equation_data = data.get("equation_dict", [])
                byte_data = data.get("byte_array_dict", {})
                validated_byte_data = validate_byte_array_dict(byte_data)
                return equation_data, validated_byte_data
...
def save_codex():
    try:
        ...
        print(f"[ERROR] save_codex failed: {e}")
...
btn_refresh = tk.Button(eq_btn_frame, text="⟳", command=refresh_all)
btn_refresh.pack(side="left", padx=5)

btn_eq_undo = tk.Button(eq_btn_frame, text="Undo", command=lambda: pop_undo(section="equation"))
btn_eq_undo.pack(side="left", padx=5)

btn_eq_redo = tk.Button(eq_btn_frame, text="Redo", command=lambda: pop_redo(section="equation"))
...
def refresh_equation_tree():
    ...
    for e in equation_dict:
        ...
        f = e.get("equation", "")
        v = ", ".join(e.get("var", []))
        b = ", ".join(byte_array_dict[byte_id]["name"] for byte_id in e.get("byte", []))  # Display names
...
def show_byte_dropdown(iid, bbox):
 ...
 # unresolved bug that happens when user clicks out of the dropdown when changes have been made to the codex.tx file. will freeze the app until user click out of the app and back in.
    def rebuild_dropdown():
        ...
        for byte_key, byte_entry in byte_array_dict.items():
            row_str = byte_entry["name"]
        ...
        close_btn = tk.Button(frame, text="Close else crash!",
...
byte_tree.heading("Data", text="Data")

# Column widths
byte_tree.column("ID", width=0, stretch=False)
byte_tree.column("Name", width=90, anchor="w")
byte_tree.column("Size", width=40, anchor="center")
byte_tree.column("Data", width=150, anchor="w")
...
def get_next_byte_id():
    global next_byte_id
    # Find the next ID that isn't used
    while str(next_byte_id) in byte_array_dict:
        next_byte_id += 1
    return str(next_byte_id)
...
def on_add_byte():
    ...
    for byte_id, byte_data in byte_array_dict.items():
        if byte_data["name"] == typed_name:
    ...
    print(f"[DEBUG] Added new byte with ID={byte_id}, name='{typed_name}'")
...
def on_byte_tree_select(event):
    ...
    byte_name = row_vals[1]  # Display name (optional for debugging)

    print(f"[DEBUG] Selected Byte ID={byte_id}, Name={byte_name}")
    ...
    if byte_id in byte_array_dict:
        ...
        # Safely remove the trace if it exists
        if hasattr(grid_size_var, 'trace_id') and grid_size_var.trace_id is not None:
            try:
                ...
                print(f"[WARNING] Could not remove trace: {e}")

        try:
            grid_size_var.set(s)  # Update grid size without triggering trace
            ...
            update_display()  # Update grid visuals
        finally:
...
def on_byte_tree_double_click(event):
    ...
    # col_id = "#1" => ID, "#2" => Name, "#3" => Size, "#4" => Data

    if col_id != "#2":  # Only rename if the user double-clicked the 'Name' column
        return
    ...
    def save_rename(*args):
        ...
        if not new_name:
            return
        # Ensure no duplicates if you prefer
        # if any(entry["name"] == new_name for entry in byte_array_dict.values()):
        #     print("[DEBUG] Duplicate name. Skipping rename.")
        #     return

        if byte_id in byte_array_dict:
            byte_array_dict[byte_id]["name"] = new_name
            print(f"[DEBUG] Renamed ID={byte_id} to name='{new_name}'")
        refresh_byte_tree()
        save_codex()
