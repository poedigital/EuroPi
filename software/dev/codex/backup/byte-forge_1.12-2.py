...
def tokenize_equation(eq_idx=None):
    """Tokenizes the equation, applies exception parsing, and manages token expansion."""
    ...
    tokens = []  # store processed tokens
    variables = []  # store detected variables
    expansion_map = {}  # store mappings of original tokens to their expansions
    ...
    for token in raw_tokens:
        ...
        if token in operator_pool["functions"]:
            ...
            if operator_pool["variables"][token].get("expandable", False):
                expansion = operator_pool["variables"][token]["expansion"]
                expansion_map[token] = expansion  # track original → expanded form
                tokens.extend(expansion)  # replace with expanded form
                continue  # do not add original token
    ...
    equation_dict[eq_idx]["expansion_map"] = expansion_map  # store expansion mapping
    equation_dict[eq_idx]["equation"] = parsed_eq

    # generate and store rpn version
    rpn_expression = infix_to_rpn(tokens, operator_pool)
    equation_dict[eq_idx]["rpn"] = rpn_expression
...
def infix_to_rpn(tokens, operator_pool):
    ...
    for token in tokens:
        ...
        if token in operator_pool["variables"] or token in operator_pool["constants"]:
            output.append(token)
            print(f"[DEBUG] Added '{token}' to RPN output")
...
def parse_exceptions(equation):
    ...
    # insert spaces around operators found in `operator_pool`
    #for op in operator_pool.get("operators", []):
