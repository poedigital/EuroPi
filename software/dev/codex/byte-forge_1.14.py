...
def load_codex():
    print("[DEBUG] load_codex() triggered.")
    ...
    try:
        ...
        operator_pool = data.get("operator_pool", default_operator_pool)

        # Ensure all required keys in operator_pool are properly initialized
        for key, default in default_operator_pool.items():
            if key not in operator_pool or not isinstance(operator_pool[key], type(default)):
                operator_pool[key] = default
...
root.geometry("624x1080") # AND HERE
...
def center_window(width=624, height=1080):
...
def on_eq_click(event):
    ...
    # Update OLED outside of the conditional
    update_oled_equation()
...
def populate_tokenizer_from_eq(eq_idx):
    ...
    for token in raw_tokens:
        ...
        # If it's unknown, apply the red background
        if assigned_token == "???":
            tag = "unknown_token"

        tokenizer_tree.insert("", "end", values=(token, category, assigned_token), tags=(tag,))

    tokenizer_tree.tag_configure("unknown_token", background="#1f0000")  # Soft red background

def validate_equation(eq):
    """
    Validates the equation for unsupported patterns, like RHS usage.
    """
    if "=" in eq:
        # Warn if equation contains an assignment operator
        print(f"[WARNING] The equation '{eq}' contains an assignment ('='). The RHS will be ignored.")

        # Optional: Raise a user-facing warning
        messagebox.showwarning(
            "Equation Warning",
            f"The equation '{eq}' contains an assignment ('='). "
            "Only the left-hand side (LHS) will be evaluated, and the result (RHS) will be ignored."
        )

def tokenize_equation(eq_idx):
    """Tokenizes the equation while ensuring exception parsing and updating the backend."""
    if eq_idx < 0 or eq_idx >= len(equation_dict):
        print("[ERROR] Invalid equation index.")
        return
    ...
    raw_tokens = parsed_eq.split(" ")  # Split into tokens
    tokenizer_tree.delete(*tokenizer_tree.get_children())  # Clear previous tokens
    ...
    for token in raw_tokens:
        ...
        if token in operator_pool.get("functions", []):
            ...
            category = "variable"
            variables.append(token)  # Add to variables

        print(f"[DEBUG] Token: '{token}', Category: '{category}'")
    ...
    if not tokens:
        print("[ERROR] No tokens found in equation.")
        return

    # Update equation_dict with tokens and variables
    eq_data["tokens"] = tokens
    eq_data["vars"] = list(set(variables))  # Remove duplicates

    # Generate RPN and save
    generate_rpn(eq_idx)
    save_codex()
...
def generate_rpn(eq_idx):
    """Generates Reverse Polish Notation (RPN) from the tokenized equation."""
    if eq_idx < 0 or eq_idx >= len(equation_dict):
        print("[ERROR] Invalid equation index.")
        set_entry_marker(False)
        return

    eq_data = equation_dict[eq_idx]
    tokens = eq_data.get("tokens", [])


    # Define operator precedence (dynamic based on operator_pool)
    operator_precedence = {
        "+": 1, "-": 1,
        "*": 2, "/": 2,
    }
    ...
    try:
        for token in tokens:
            if token == "=":
                # Skip the assignment operator as it's not part of RPN
                print(f"[DEBUG] Skipping token: '{token}'")
                continue
            elif token in operator_pool["functions"]:
                operator_stack.append(token)  # Functions go to the operator stack
            ...
            elif token in operator_pool["groupings"]:  # Handle parentheses
                if token == "(":
                    operator_stack.append(token)
                elif token == ")":
            ...
            elif token in operator_pool["variables"] or token.isalnum():
                output_queue.append(token)  # Variables or literals directly to output
            else:
        ...
        # Save RPN to the equation dict
        eq_data["rpn"] = output_queue
        print(f"[DEBUG] RPN for equation {eq_data['title']}: {output_queue}")
        save_codex()  # Ensure persistence

        # Set green marker for success
        set_entry_marker(True)

    except Exception as e:
        print(f"[ERROR] Failed to generate RPN: {e}")
        set_entry_marker(False)  # Set red marker

def set_entry_marker(is_valid):
    """Sets a red border on the tokenizer entry if there's an error. Clears it otherwise."""
    if is_valid:
        # Set normal border color (black is safe for most themes)
        tokenizer_entry.config(highlightbackground="black", highlightcolor="black", highlightthickness=1)
    else:
        # Set red border for errors
        tokenizer_entry.config(highlightbackground="red", highlightcolor="red", highlightthickness=2)
...
def move_token(direction):
    ...
    # Update the text in the entry field
    tokenizer_entry.delete(0, tk.END)  # Clear the entry
    tokenizer_entry.insert(0, " ".join(new_tokens))  # Insert the updated tokens
...
def refresh_ranges_tree():
    ...
    # Ensure 'settings' and 'ranges' exist
    if "settings" not in eq_data:
        eq_data["settings"] = {}
    if "ranges" not in eq_data["settings"]:
        eq_data["settings"]["ranges"] = {}

    # Sync ranges with current variables
    vars_list = eq_data.get("vars", [])
    eq_data["settings"]["ranges"] = {
        v: eq_data["settings"]["ranges"].get(v, [0.1, 2.0]) for v in vars_list
    }

    # Populate the ranges tree
    for v in vars_list:
...
def on_ranges_double_click(event):
    ...
    if not row_vals or len(row_vals) != 3:
        print(f"[ERROR] Incomplete or missing row values: {row_vals}")
        return
    ...
    # We only want to let them edit min or max, i.e., columns #1 or #2 in 0-based indexing
    if c_index == 0:
        return  # "Var" column is read-only
    ...
    def save_edit(event=None):
        ...
        try:
            ...
            print("[ERROR] Invalid value entered, reverting to old value.")
        ...
        # Update the range
        if c_index == 1:  # Min
            var_ranges[0] = new_val
        elif c_index == 2:  # Max
            var_ranges[1] = new_val

        # Update the row in the TreeView and save
        updated_row = list(row_vals)
        updated_row[c_index] = new_val
        ranges_tree.item(iid, values=tuple(updated_row))
        save_codex()

    # Bind events for saving
...
def add_operator(pre_filled_symbol=None):
    ...
    # Shift the popup 100px to the left and 30px up
    shift_x = 150
    shift_y = 48
    new_x = cursor_x - shift_x
    new_y = cursor_y - shift_y

    add_window.geometry(f"+{new_x}+{new_y}")
    # Category Selection
    tk.Label(add_window, text="Category:").grid(row=0, column=0, padx=5, pady=5, sticky="e")
    ...
    # Symbol/Key Entry
    tk.Label(add_window, text="Symbol/Key:").grid(row=1, column=0, padx=5, pady=5, sticky="e")
    symbol_entry = tk.Entry(add_window)
    symbol_entry.grid(row=1, column=1, padx=5, pady=5, sticky="w")
    if pre_filled_symbol:
        symbol_entry.insert(0, pre_filled_symbol)

    # Value Entry (Optional)
    tk.Label(add_window, text="Value (Optional):").grid(row=2, column=0, padx=5, pady=5, sticky="e")
    ...
    def confirm_add():
        ...
        # Initialize category in operator_pool if it doesn't exist
        if category not in operator_pool:
            operator_pool[category] = {} if category == "constants" else []

        # Handle addition based on category type
        ...
        # Save and refresh the operator pool
        save_codex()
        refresh_operator_tree()
        refresh_ranges_tree()

        # Re-tokenize the current equation if any is selected
        eq_sel = eq_tree.selection()
        if eq_sel:
            eq_idx = eq_tree.index(eq_sel[0])
    ...
    # Add Button
    tk.Button(add_window, text="Add", command=confirm_add).grid(row=3, column=0, columnspan=2, pady=10)

operator_tree.bind("<Button-3>", lambda event: add_operator())
