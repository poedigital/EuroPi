...
def output_notes():
    ...
    for output, scale_name in OUTPUT_SCALES.items():
        ...
        if plane_number > 1:
            interval += get_plane_interval(plane_number)
...
def randomize_planes(set_display_time=True):
    ...
    for plane_number in range(2, planes + 1):
        ...
        possible_intervals = list(PLANE_INTERVALS.values())
...
def init_patch():
    global planes, current_step_index, current_note_index, plane1_notes, plane_notes, last_note_value
    planes = 1
...
def main():
    global planes, current_step_index, current_note_index
