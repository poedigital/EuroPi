...
def randomize_planes(set_display_time=True):
    ...
    # Generate new sequence steps with shuffling
...
def init_patch():
    ...
    plane1_notes[0] = 0
    reference_note1_value = plane1_notes[0]
    plane_notes = PLANE_INTERVALS.copy()  # Use default intervals

    # Do not randomize planes or sequence steps at initialization
    # randomize_planes(set_display_time=False)

    update_last_active_note_index()

    # Generate initial sequence steps without shuffling
    generate_sequence_steps()
