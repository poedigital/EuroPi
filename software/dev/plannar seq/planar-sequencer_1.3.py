...
class PlanarSequencer(EuroPiScript):
    def __init__(self):
        ...
        KNOB_HYSTERESIS = 2      # Hysteresis for k1 and k2
        AIN_HYSTERESIS = 0.02   # Hysteresis for ain  
    ...
    def apply_hysteresis(self, current_value, last_value, threshold):
        """
        Apply hysteresis to smooth input changes.

        Args:
            current_value (float): Current input value.
            last_value (float): Last stable input value.
            threshold (float): Hysteresis threshold.

        Returns:
            tuple: (bool, new_value) - Whether the value changed and the new stable value.
        """
        if abs(current_value - last_value) > threshold:
            return True, current_value  # Value has changed significantly
        return False, last_value  # No significant change
    ...
    def read_root_note(self):
        ...
        changed, stable_voltage = self.apply_hysteresis(voltage, self.last_ain_voltage, self.AIN_HYSTERESIS)
        if changed:
            self.last_ain_voltage = stable_voltage
            if self.ain_mode == 'ROOT':
                midi_note = int((stable_voltage / 5.0) * 60) + 36
                ...
                max_position = 16
                position = int((stable_voltage / 5.0) * max_position) % max_position
                ...
                midi_note = int((stable_voltage / 5.0) * 60) + 36
                return midi_note
        return self.last_root_note  # Return the last stable root note
    ...
    def handle_k1_input(self, k1_value, current_time):
        changed, stable_value = self.apply_hysteresis(k1_value, self.last_k1_value, self.KNOB_HYSTERESIS)
        if changed:
            max_position = 16
            position = int((stable_value / 100) * (max_position + 1))
            ...
            self.update_last_active_note_index()
            self.last_k1_value = stable_value  # Update last stable value
            self.update_display()
            self.last_activity_time = current_time
            self.request_save_current_state()

    def handle_k2_input(self, k2_value, current_time):
        changed, stable_value = self.apply_hysteresis(k2_value, self.last_k2_value, self.KNOB_HYSTERESIS)
        if changed:
            new_planes = int(round((stable_value / 99.0) * (self.MAX_PLANES - 1))) + 1
            ...
            if new_planes != self.planes:
                ...
                self.last_activity_time = current_time
                self.generate_sequence_steps()
            self.last_k2_value = stable_value  # Update last stable value
            self.request_save_current_state()
